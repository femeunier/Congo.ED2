...
plot(dataC02.all$year,
     dataC02.all$CO2,xlim = c(1950,2023))
...
for (cmodel in models){
  ...
  file1 <- paste0("./data/grid.",cmodel,".recentyears.RDS")
  file2 <- paste0("./data/grid.JRA.",cmodel,".RDS")
  ...
  for (i in seq(1,nrow(xgb.models))){
    ...
    predicted.JRA <- predict(cXGBmodel,
                              cdf.JRA[,cXGBmodel$finalModel$feature_names])
    ...
    all.predicted.JRA <- inputs.JRA %>%
      ...
      dplyr::select(c("biome","model","continent","xgb.model","year",
                      ...
                      "month","lat","lon")) %>%
      mutate(var = cvar,
             pred.JRA = predicted.JRA)


    cpredicted <- all.predicted %>%
      full_join(all.predicted.JRA,
  ...
  saveRDS(df.all,
          paste0("./outputs/predictions.XGB.",cmodel,".JRA.RDS"))
}
...
# scp /home/femeunier/Documents/projects/Congo.ED2/scripts/test.XGBmodels.JRA.R hpc:/data/gent/vo/000/gvo00074/felicien/R/
