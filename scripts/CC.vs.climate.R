...
all.models <- all.grids <- data.frame()
...
for (cmodel in models){
  ...
  if (!all(file.exists(model.file,grid.file))){
    next()
  }
...
for (cmodel in models){
  ...
  for (cvar in vars){
    ...
    op.file <- paste0("./outputs/",
                      "xgb_model.time.latlon.Amazon.",cmodel,".",cvar,".RDS")
...
# scp /home/femeunier/Documents/projects/Congo.ED2/scripts/CC.vs.climate.R hpc:/data/gent/vo/000/gvo00074/felicien/R
