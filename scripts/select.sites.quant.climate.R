...
df.MAP <- readRDS(file = "/home/femeunier/Documents/projects/YGB/data/Precip/df.precip.RDS")
...
#########################################################
# SW
df.sw <- readRDS("/home/femeunier/Documents/projects/YGB/data/SW/CERES/df.CERES.RDS") %>%
...
if (precip.source == "all"){
  df.MCWD <- readRDS("/home/femeunier/Documents/projects/YGB/data/MCWD/df.MCWD.RDS") %>%
    filter(!is.na(MCWD)) %>%
...
select.sites <- df.long.med %>%
  ungroup() %>%
  dplyr::select(lon,lat,var,value,AGB) %>%
  distinct()
