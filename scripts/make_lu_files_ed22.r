...
inpdir  = "/home/femeunier/Documents/projects/Congo.ED2/data/LUH"  # Path with inputs
histdir = file.path(inpdir,"historical")                  # Input path

#----- Future land use scenarios. ---------------------------------------------------------#
# rcp     = c("rcp26_image","rcp45_minicam","rcp60_aim","rcp85_message")
rcp = "scenario"
#------------------------------------------------------------------------------------------#
...
#------------------------------------------------------------------------------------------#
lona    = -15.   # Westernmost edge (first block longitude will be lona + dblock/2)
lonz    =  50.   # Easternmost edge (the closest that creates regular blocks will be used)
lata    =  -20.  # Southernmost edge (first block latitude will be lata + dblock/2)
latz    =  15.   # Northernmost edge (the closest that creates regular blocks will be used)
dblock  =  0.5   # Size of block (both longitude and latitude)
#------------------------------------------------------------------------------------------#
...
mindbh.slog   = ifelse(tropical,0.0, 0.0)
...
#----- Load some scripts. -----------------------------------------------------------------#
source(file.path(here,"scripts","solid.angle.r"))
#------------------------------------------------------------------------------------------#
...
outmain = file.path(here,"outputs",outpref)
if (dblock == 1.0){
...
df.disturb <- as.data.frame(disturb) %>%
...
ggplot(data = df.disturb) +
  geom_line(aes(x = year, y = value)) +
  facet_wrap(~var,scales = "free_y") +
  theme_bw()


important.transitions <- df.disturb %>%
  filter(var %in% c("cp","pc","pv","vp","vc","cv","sc","cs","sp","ps","vs")) %>%
  group_by(var) %>%
  summarise(value.max = max(value),
            .groups = "keep") %>%
  arrange(desc(value.max)) %>%
  ungroup() %>%
  slice_head(n = 5)

ggplot(data = df.disturb %>%
         filter(var %in% c("cp","pc","pv","vp","vc","cv","sc","cs","sp","ps","vs")) %>%
         filter(var %in% (important.transitions %>% pull(var)))) +
