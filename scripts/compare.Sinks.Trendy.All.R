...
woody.ratio.of.npp <-
  data.frame(model = c("model.name"),
             woody.ratio.of.npp = c(NA))
...
npp.trendy.Amazon <- npp.trendy %>%
  ungroup() %>%
  dplyr::filter(lon >= -80,lon <= -35)

npp.trendy.Africa <- npp.trendy %>%
  ungroup() %>%
  dplyr::filter(lon >= -12,lon <= 45)

npp.trendy.Asia <- npp.trendy %>%
  ungroup() %>%
  dplyr::filter(lon >= 75)
...
df.sink.regrid <- df.npp.regrid <- df.AGB.regrid <- data.frame()
for (cmodel in unique(Biomass.Trendy.mean.sink[["model"]])){
  ...
  npp.regrid <- resample.df.all.col(bigdf = npp.trendy.mean %>%
                                      ungroup() %>%
                                      filter(model == cmodel) %>%
  ...
  df.sink.regrid <- bind_rows(list(df.sink.regrid,
                                   Biomass.Trendy.mean.sink.regrid %>%
                                     mutate(model = cmodel)))
...
for (iplot in seq(1,length(all.plots))){
  ...
  for (isubplot in seq(1,nrow(cdf))){
    ...
    cdf.flux.sum <- process.Trendy.sum(Biomass.Trendy = cccBiomass.Trendy.search,
                                       NPP.trendy = cccnpp.trendy,
                                       target = c(start.date.int,
                                                  end.date.int),
                                       params = list(default = default.woody.ratio.of.npp,
                                                     model.params = woody.ratio.of.npp)) %>%
      mutate(Continent = cContinent,
             plot = cplot,
             int_ini = start.date,
             int_fin = end.date,
...
saveRDS(df.model,
        "./outputs/Model.vs.Data.Trendy.RDS")
...
df2plot <- df.model.filtered %>%
  dplyr::select(model,Continent,
                obsSink,obsGrowth,obsMort,
                sink.corrected,Growth,Mort) %>%
  rename(Obs_Sink = obsSink,
         Obs_Growth = obsGrowth,
         Obs_Mort = obsMort,
...
df.model.sum <- df.model.long.filtered %>%
  ...
  summarise(cAGB.m = weighted.mean(cAGB,1/dist,
            ...
            AGBGain_tot.ha.yr = unique(obsGrowth),
            AGBMor_tot.ha.yr = unique(obsMort),
            .groups = "keep") %>%
  ungroup() %>%
...
df.model.sum.time <- df.model.sum %>%
  ...
  summarise(modeled.sink = weighted.mean(modeled.sink,weight,na.rm = TRUE),
            ...
            modeled.growth = weighted.mean(modeled.growth,weight,na.rm = TRUE),
            observed.growth = weighted.mean(observed.growth,weight,na.rm = TRUE),

            modeled.mort = weighted.mean(modeled.mort,weight,na.rm = TRUE),
            observed.mort = weighted.mean(observed.mort,weight,na.rm = TRUE),
...
df.model.sum.time.all.long <- df.model.sum.time.all %>%
  dplyr::select(-c(cAGB)) %>%
  pivot_longer(cols = c(modeled.sink,observed.sink,modeled.growth,observed.growth,modeled.mort,observed.mort),
               names_to = "variable",
               values_to = "value") %>%
...
ggplot() +
  ...
  geom_line(data = data.observed %>%
              filter(variable %in% c("growth","mort","sink")),
            aes(x = year,
...
saveRDS(df.model.sum.time.all.long %>%
          dplyr::filter(type == "modeled",
                        variable == "sink",
                        model == "Model Ensemble Mean"),
        "./outputs/Model.ensemble.inventories.RDS")

ggplot() +

  geom_line(data = df.model.sum.time.all.long %>%
              filter(variable == "sink",type == "modeled" & model != "Model Ensemble Mean"),
            aes(x = year,
                y = value,
                color = as.factor(model))) +
  geom_line(data = df.model.sum.time.all.long %>%
...
ggplot() +
  ...
  geom_line(data = df.model.sum.time.all.long %>%
              filter(Continent %in% c("Amazon","Africa","Asia"),
                     variable == "sink",type == "modeled" & model != "Model Ensemble Mean"),
            aes(x = year,
                y = value,
                color = as.factor(model))) +
  geom_line(data = df.model.sum.time.all.long %>%
              dplyr::filter(type == "modeled",
                            Continent %in% c("Amazon","Africa","Asia"),
                            variable == "sink",
                            model == "Model Ensemble Mean"),
            aes(x = year,
                y = value),color = "black",linetype = 1) +
...
ggplot() +
  ...
  geom_hline(yintercept = 0, linetype = 3) +
  facet_grid( ~ Continent) +
  labs(x = "", color = "", y = "Carbon stocks (kgC/m²)") +
  theme_bw()
...
ggplot() +
  geom_density(data = df.model.sum.long.all.fltrd %>% filter(type.var == "modeled",
                                                             ...
                                                             Continent %in% c("Amazon","Africa","Asia")) %>%
                 mutate(Continent = relevel(Continent,"Amazon")),
  ...
  geom_density(data = df.model.sum.long.all.fltrd %>% filter(model == model[1],
               ...
               alpha = 0.5) +
  facet_wrap(Continent ~ type.obs, scales = "free", nrow = 1) +
  labs(x = "C flux (MC/ha/yr)") +
  theme_bw() +
  guides(fill = "none")

diffs <- df.model.sum.long.all.fltrd %>%
  filter(type.obs %in% c("sink"),
         Continent %in% c("Amazon","Africa","Asia")) %>%
  group_by(model,Continent,type.var) %>%
  summarise(m = mean(value,na.rm = TRUE),
            .groups = "keep") %>%
  pivot_wider(names_from = c("type.var"),
              values_from = m) %>%
  mutate(diff = observed - modeled)

ggplot(data = diffs) +
  geom_density(aes(x = diff, fill = Continent),
               alpha = 0.5) +
...
ggplot() +
  geom_density(data = df.model.sum.long.all.fltrd %>% filter(type.var == "modeled",
                                                             type.obs %in% c("sink","growth","mort"),
                                                             model != "Model Ensemble"),
               aes(x = value, fill = model), color = NA,
               alpha = 0.5) +
...
ggplot() +
  ...
  geom_hline(color = "black", linetype = 2, yintercept = 0) +

  scale_fill_manual(values = c("darkgrey",hex <- hue_pal()(16))) +
  scale_x_continuous(breaks = 2:(1+length(levels(model2plot$model))),
...
#   group_by(type.var,Continent,type.obs) %>%
#   summarise(m = mean(value),
#             med = median(value)) %>%
#   arrange(type.obs,Continent)
#
#
# df.model.sum %>%
#   group_by(model) %>%
#   summarise(modeled.sink.m = mean(modeled.sink,na.rm = TRUE),
#             observed.sink.m = mean(observed.sink,na.rm = TRUE)) %>%
#   dplyr::select(model,modeled.sink.m,observed.sink.m) %>%
#   arrange(desc(modeled.sink.m))
#
#
# ggplot(data = df.model.sum,
#        aes(x = modeled.sink, y = observed.sink, color = Continent)) +
#   geom_point() +
#   geom_abline(slope = 1,
#               intercept = 0) +
#   stat_smooth(method = "lm",
...
#
# ggplot(data = df.model.sum,
#        aes(x = modeled.growth, y = observed.growth, color = Continent)) +
#   geom_point() +
#   geom_abline(slope = 1,
...
#        aes(x = modeled.growth.m, y = observed.growth, color = Continent)) +
#   geom_point() +
#   geom_abline(slope = 1,
#               intercept = 0) +
#   stat_smooth(method = "lm",
...
#
# ggplot(data = df.all.model.sum,
#        aes(x = modeled.sink.m, y = observed.sink, color = Continent)) +
#   geom_point() +
#   geom_abline(slope = 1,
#               intercept = 0) +
#   stat_smooth(method = "lm",
