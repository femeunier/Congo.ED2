...
for (i in seq(1,nrow(df.mask))){
  ...
  if (nrow(details.file) > 0){
    ...
    post.last <- which.max((years.S.files) + (months.S.files)/12)

    file2check <- files.names[which(grepl("1850",files.names))]

    if (length(file2check) > 0){
      if (any(month(file.info(file2check)$ctime) >= Inf)) next()
    }
  ...
  write_ed2in(ed2in_scenar,filename = file.path(run_ref,"ED2IN_restart_Efile"))
...
dumb <- write_bash_submission(file = file.path(rundir,"all_jobs_restart_Efiles.sh"),
                              list_files = list_dir,
                              job_name = "job.sh")

# scp /home/femeunier/Documents/projects/Congo.ED2/scripts/restart_CB_simulations.R hpc:/data/gent/vo/000/gvo00074/felicien/R
