...
for (i in seq(1,nrow(df.mask))){
  ...
  if (nrow(details.file) > 0){
    ...
    file2check <- files.names[which(grepl(as.character(end.year),files.names))]

    while (end.year > 1550 &
           (!(( any(month(file.info(file2check)$ctime) == 12 & year(file.info(file2check)$ctime) == 2022)) |
              any(year(file.info(file2check)$ctime) == 2023)))
    ) {
      end.year <- end.year - 1
      file2check <- files.names[which(grepl(as.character(end.year),files.names))]
    }
    ...
    for (year in unique(c(seq(1550,last.year,50),last.year))){

      h5file <- file.path(dirname(files.names)[1],
                               paste("history","S",as.character(year),"01-01-000000-g01.h5",sep = "-"))
      ...
      cdf <- bind_rows(list(cdf,
                            data.frame(lat = clat,
                                       ...
                                       LAI.trees = sum(mymont$LAI.PY[1,,2:4]),
                                       LAI.grass = sum(mymont$LAI.PY[1,,1]),
                                       GPP = sum(mymont$GPP))
                            ))
    }
...
saveRDS(df,"df_CB.RDS")

# scp /home/femeunier/Documents/projects/Congo.ED2/scripts/analyze_CB.R hpc:/data/gent/vo/000/gvo00074/felicien/R
