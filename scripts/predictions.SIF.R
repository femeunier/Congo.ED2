...
for (cproduct in products){
  ...
  xgb_best_model <- readRDS(paste0("./outputs/",cproduct,".vs.climate.Amazon.ILF.RDS"))
...
saveRDS(df.test.pred,
        "./outputs/test.predictions.SIF.ILF.RDS")

saveRDS(df.all.pred,
        "./outputs/all.predictions.SIF.ILF.RDS")

saveRDS(df.all.pred.sum,
        "./outputs/all.predictions.SIF.sum.ILF.RDS")
