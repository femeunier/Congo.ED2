...
plot(dataC02.all$year,
     dataC02.all$CO2,xlim = c(1950,2025))
...
for (cmodel in models){
  ...
  file1 <- paste0("./data/grid.",cmodel,"allyears.RDS")
  file2 <- paste0("./data/grid.ERA5.",cmodel,".RDS")

  if (any(!file.exists(file1,file2))){
    next()
  }

  climate <- readRDS(file1)
  ERA5 <- readRDS(file2)
  ...
  inputs <- climate %>%
    ...
    mutate(xgb.model = paste0("XGB.fit.allyears.",model,".S2.",continent,".",biome)) %>%
  ...
  for (i in seq(1,nrow(xgb.models))){
    ...
    cxgb.model.novar <- xgb.models[["model"]][i]
    ...
    predicted.ERA5 <- predict(cXGBmodel,
                              cdf.ERA5[,cXGBmodel$finalModel$feature_names])
    ...
    all.predicted.ERA5 <- inputs.ERA5 %>%
      filter(xgb.model == cxgb.model.novar) %>%
      filter(model.lat.lon %in% all.data[["model.lat.lon"]]) %>%
      dplyr::select(c("biome","model","continent","xgb.model","year",
                      "model.lat.lon",
                      "month","lat","lon")) %>%
      mutate(var = cvar,
             pred.ERA5 = predicted.ERA5)


    cpredicted <- all.predicted %>%
      full_join(all.predicted.ERA5,
                by = c("biome","model","continent","xgb.model","year","month","var",
                       "model.lat.lon","lat","lon")) %>%
      left_join(all.data %>%
  ...
  saveRDS(df.all,
          paste0("./outputs/predictions.XGB.",cmodel,"ERA5.RDS"))
}

# saveRDS(df.all,
#         "./outputs/predictions.XGB.all.years.RDS")


# scp /home/femeunier/Documents/projects/Congo.ED2/scripts/test.XGBmodels.ERA5.R hpc:/data/gent/vo/000/gvo00074/felicien/R/
