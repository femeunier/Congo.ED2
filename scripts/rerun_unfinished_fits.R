...
for (irow in seq(1,nrow(grid))){
  ...
  op.file <- paste0(xgb.model.prefix,"*.RDS")
  exist.files <- list.files(file.path(main.dir,'outputs'),
                            pattern = paste0(xgb.model.prefix,"*"))

  if (length(exist.files) == 0){
    vars2rerun <- all.vars
  } else {
    vars2rerun <- all.vars[! (paste0(xgb.model.prefix,".",
                                     all.vars,".RDS") %in% exist.files)]
  }

  if (length(vars2rerun) == 0) next()
  ...
  write.script.fit(
    ...
    xgb.model.prefix = xgb.model.prefix,
...
# scp /home/femeunier/Documents/projects/Congo.ED2/scripts/rerun_unfinished_fits.R hpc:/kyukon/data/gent/vo/000/gvo00074/felicien/R/
