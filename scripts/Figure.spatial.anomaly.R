...
mean.months <- A.MEM.ts.dt %>%
  ...
  summarise(mean.month = mean(pred.m - (slope*(time) +intercept)),
            sd.anomaly = sd((pred.m - (slope*(time) +intercept))),
            .groups = "keep")
...
slope <- unique(B.MEM.ts.dt$slope) ; intercept <- unique(B.MEM.ts.dt$intercept)
mean.months <- B.MEM.ts.dt %>%
  group_by(year,month) %>%
  summarise(pred.m = mean(pred.m),
            .groups = "keep") %>%
  mutate(time = year + (month -1/2)/12) %>%
...
all.MEM.dt %>%
  filter(year == 2023,
         month == 10) %>%
  group_by(source) %>%
  summarise(frac = sum(anomaly.m < -5)/length(anomaly.m)*100,
            .groups = "keep") %>%
  arrange(desc(frac)) %>%
  group_by(source) %>%
  slice_head(n = 10)
...
ggplot(data = all.MEM.dt.groups.sum,
  ...
  geom_density_ridges_gradient(show.legend = FALSE, alpha = 1,
                               scale = .9) +
  scale_fill_gradient2(limits = c(-2,1)*5,
                       oob = scales::squish,
                       midpoint = 0,
                       low = "darkred",mid = "grey",high = "darkgreen") +
  geom_vline(xintercept = 0,linetype = 2) +
