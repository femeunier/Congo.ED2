...
for (iphen in seq(1,length(phen_schemes))){
  for (i in seq(1,nrow(df.mask))){
    ...
    ed2in_scenar$IPHEN_SCHEME <- phen_schemes[iphen]
...
dumb <- write_bash_submission(file = file.path(rundir,"select_jobs.sh"),
                              list_files = list_dir,
                              job_name = "job.sh")

# scp /home/femeunier/Documents/projects/Congo.ED2/scripts/generate_selected_sites_CB_phen.R hpc:/data/gent/vo/000/gvo00074/felicien/R
